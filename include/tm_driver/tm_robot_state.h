...
private:
	...
	static std::vector<double> rads(std::vector<double> degs)
	{
		std::vector<double> rv(degs.size());
	...
	static std::vector<double> si_pose(std::vector<double> src)
	{
		std::vector<double> rv(src.size());
		for (size_t i = 0; i < 3; ++i) { rv[i] = src[i]; }
		for (size_t i = 3; i < src.size(); ++i) { rv[i] = rad(src[i]); }
		return rv;
	}
	template<typename T>
...
public:
	void deserialize_json(json json);
