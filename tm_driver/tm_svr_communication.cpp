...
void TmSvrCommunication::clean_json(std::string* data, int n)
{
	smatch cm;
	regex json_bounds("[\\[][{].*?([}][\\]])");

	regex_search(*data, cm, json_bounds);
	...
	regex json_item("[\"]Item[\"][:]");
	regex json_value(",[\"]Value[\"]");
	regex json_obj("[}],[{]");

	*data = regex_replace(*data, json_item, "");
	*data = regex_replace(*data, json_value, "");
	*data = regex_replace(*data, json_obj, ",");
		...
		if (pack.type == TmPacket::Header::CPERR) {
			...
			if (data.is_valid()) {
				...
				case TmSvrData::Mode::BINARY:
					...
					break;
				case TmSvrData::Mode::JSON:
					s = data.content();
					clean_json(&s, data.data_size());
					if (s.empty()) break;

					j = json::parse(s, nullptr, false);
					if (j.type() == nlohmann::detail::value_t::discarded) {
						print_info("JSON DATA ERROR\r\n");
						break;
					}
					state.deserialize_json(j);
					//delete &s;
					break;
				case TmSvrData::Mode::STRING:
					break;
