...
void TmRobotState::deserialize_json(json json)
{
	mtx_lock();
	_is_linked = json["Robot_Link"].get<unsigned char>();
	_has_error = json["Robot_Error"].get<bool>();
	_is_proj_running =  json["Project_Run"].get<bool>();
	_is_proj_paused = json["Project_Pause"].get<bool>();
	//_is_proj_edit_ = json["Project_Edit"].get<bool>();
	_is_safeguard_A_triggered = json["Safeguard_A"].get<bool>();
	_is_ESTOP_pressed = json["ESTOP"].get<bool>();
	_camera_light = json["Camera_Light"].get<char>();
	_camera_light = json["Camera_Light"].get<char>();

	_error_code = json["Error_Code"].get<int>();

	if (use_radians) {
		_joint_angle = rads(json["Joint_Angle"].get<std::vector<double>>());
		_flange_pose = si_pose(json["Coord_Robot_Flange"].get<std::vector<double>>());
		_tool_pose = si_pose(json["Coord_Robot_Tool"].get<std::vector<double>>());
	}
	else{
		_joint_angle = json["Joint_Angle"].get<std::vector<double>>();
		_flange_pose = json["Coord_Robot_Flange"].get<std::vector<double>>();
		_tool_pose = json["Coord_Robot_Tool"].get<std::vector<double>>();
	}

	_tcp_force_vec = json["TCP_Force"].get<std::vector<double>>();

	_tcp_force = json["TCP_Force3D"].get<double>();
	mtx_unlock();
}
